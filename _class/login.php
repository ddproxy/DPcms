...
class user {
	...
	public function pass_cms($cms) {
		$this->ddp = $cms;
	}
	public function login() {
			$this->username = stripslashes($_POST['username']);
			$this->password = md5($_POST['password']);
			$u_q = $this->ddp->query("SELECT * FROM users WHERE username='$this->username' AND password='$this->password' LIMIT 1");
			$u = $this->ddp->fetch_assoc($u_q);
			if($u['id']) {
				$_SESSION['id'] = $u['id'];
				$_SESSION['ddp_admin'] = $u['access_level'];
				$_SESSION['started'] = time();
				foreach($u as $key => $value)
					$_SESSION[$key] = stripslashes($value);
				$this->last_login = date('c',time());
				// on successful login update users last login
				$this->ddp->query("UPDATE users SET last_login='$this->last_login' WHERE id='".$_SESSION['id']."' LIMIT 1"); 
	...
	private function logout_form() {
		...
		<br><form id="loginform" action="index.php" method="post" name="loginform">
...
LOGOUT_FORM;
	...
	private function login_form() {
		...
		<form id="loginform" action="index.php" method="post" name="loginform">
